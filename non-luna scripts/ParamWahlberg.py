...
if Parameter=='qmin':
	...
	xaxis = r'$q_{min}$'
if Parameter=='mach':
	ParamArray = np.linspace(0.0,0.30,10) #originally 30
	xaxis = r'$v_0/v_a$'
...
for x in ParamArray:
	...
	#Boundary
	#====================================================================
	epsilon_a = 0.3 # changed from 0.3
	mu0 = 4.*np.pi*1.0E-07
	r0 = 1.0
	R0 = r0/epsilon_a
	#R0 = 3.3333333
	...
	if RunStab:
		...
		if len(Gamma2Norm) >= 2:
			...
			EV_guess += (1.0j)*EV_guess.imag*0.001 # play with this number, may need to be larger to be further to the right
		else:
	...
	#=====================================================     
		if EV < 1E-7:
                        ...
                        Gamma2NormGuess.append(EV_guess)
                        break
...
Gamma2NormGuess = np.asarray(Gamma2NormGuess,dtype=complex)
...
#plt.plot(V0_VA,Gamma2NormGuess.real,'x-',label=r'EV guess')
plt.xlabel('v0/va')
